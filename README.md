...
Blank edit instructions rotate through five prompt families—clean removal, color/material change, addition, swap/replacement, and a gently silly transformation—twice across a ten-region puzzle. Automatic edits prioritize one discrete non-anatomical element near the crop center, forbid facial reconstruction and beautification, and fall back to adding a harmless obvious detail when no safe removable element exists. Every result is instructed to contain one unmistakable difference visible at normal full-image scale.
