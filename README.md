...
Blank edit instructions rotate through ten broad creative directions, including subtraction, state, appearance, expression/anatomy, physical addition, replacement, count, proportion, arrangement, and an open-ended visual surprise. They are intentionally suggestive rather than prescriptive: ChatGPT chooses an image-specific subject and changes direction when a family does not fit the crop. Faces, expressions, hair, facial features, bodies, objects, and surfaces are all valid targets. Every result must contain one unmistakable difference visible at normal full-image scale.

AI mode presents a three-step creator workflow: upload one photo, generate the complete puzzle, then download the modified PNG and config JSON together. Region editing, retry controls, individual downloads, import tools, and diagnostics remain available under Advanced options. Automatic edits favor integrated changes to the photographed reality—removing, duplicating, reshaping, recoloring, rearranging, changing state or count, or transforming anatomy and expression—rather than stickers, decals, icons, or pasted-on graphics. Anatomical edits remain clean, harmless, and non-graphic.
...
Creator exports version 3 JSON containing the source image dimensions, number of required finds, and normalized clickable rectangle regions. The player also accepts optional reveal bounds from older AI-generated configs and migrates older version 2 circle-based configs when loaded.
