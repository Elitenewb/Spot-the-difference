...
6. Select **Generate pending edits**. The extension opens a dedicated ChatGPT tab and processes one padded crop in a fresh temporary conversation at a time. When ChatGPT returns an image card, the extension opens its full-size viewer and reads the resulting asset back into the creator.
...
The full image is uploaded to ChatGPT only for region analysis. Analysis and each image-editing request use a fresh temporary chat, so the automation does not add conversations to ChatGPT history. Each edit contains only one padded crop. Returned crops are resized to their original crop geometry, clipped to a modest safe area around the selected rectangle, feathered at the edges, and composited on a canvas with the original image dimensions. The extra compositing room lets a connected feature such as a hat finish naturally across the edge of the puzzle's clickable rectangle.
