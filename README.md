...
Blank edit instructions use one concise rule: make a single clear, fun, reality-changing difference that will still be noticeable when the crop is placed back into the full image. ChatGPT chooses the target that best fits the crop; faces, expressions, hair, anatomy, objects, and scenery are all valid. Automatic edits reject stickers, labels, and graphic overlays while preserving the crop framing and unrelated content.

AI mode presents a three-step creator workflow: upload one photo, generate the complete puzzle, then download the modified PNG and config JSON together. Region editing, retry controls, individual downloads, import tools, and diagnostics remain available under Advanced options. Optional instructions can still name one exact change for a selected crop.
