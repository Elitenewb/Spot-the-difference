...
test('creator and adapter include recovery guards for stalled website automation', () => {
  ...
  assert.ok(creator.includes('TOOL POLICY: Do not call, open, or hand this task to Adobe, Photoshop, Canva'));
  assert.ok(creator.includes('Do not ask for tool permission'));
  ...
  assert.ok(creator.includes('Explore subtraction'));
  assert.ok(creator.includes('Explore expression or anatomy'));
  assert.ok(creator.includes('Explore number or repetition'));
  assert.ok(creator.includes('Explore shape, scale, or proportion'));
  assert.ok(creator.includes('Explore an original visual surprise'));
  assert.ok(creator.includes('A near-identical output, subtle retouch, general cleanup'));
  assert.ok(creator.includes('Human and animal anatomy is explicitly welcome'));
  assert.ok(creator.includes('Closing or cleanly erasing eyes'));
  assert.ok(creator.includes('removing or reforming a facial feature'));
  assert.ok(creator.includes('changing a hairstyle'));
  assert.ok(creator.includes('alter the depicted reality itself rather than decorating the image'));
  assert.ok(creator.includes('Do not add a sticker, decal, badge, icon, emoji'));
  assert.ok(!creator.includes('non-anatomical'));
  ...
  assert.ok(creator.includes('CUSTOM DIFFERENCE: ${instruction}'));
  ...
  assert.ok(creator.includes('const sourceX=x,sourceY=y,sourceW=w,sourceH=h'));
  assert.ok(creator.includes('drawImage(state.workCanvas,geometry.sourceX,geometry.sourceY,geometry.sourceW,geometry.sourceH'));
  ...
  assert.ok(creator.includes('The uploaded image is exactly the user-selected crop'));
  assert.ok(creator.includes('the whole crop is available for editing'));
  assert.ok(creatorHtml.includes('creator.js?v=20'));
  ...
  assert.ok(adapter.includes('TOOL POLICY: Do not call, open, or hand this task to Adobe, Photoshop, Canva'));
  assert.ok(adapter.includes('Alter the depicted reality itself through a physical, structural, material, state, count, expression, or anatomical change'));
  assert.ok(adapter.includes('do not add a sticker, decal, badge, icon, emoji'));
