...
test('creator and adapter include recovery guards for stalled website automation', () => {
  ...
  assert.ok(creator.includes('smallest clearly removable non-anatomical element'));
  ...
  assert.ok(creator.includes('swapping one existing non-anatomical item, letter, number, symbol'));
  assert.ok(creator.includes('gently silly transformation'));
  assert.ok(creator.includes('A near-identical result, subtle retouch, general cleanup'));
  assert.ok(creator.includes('do not remove, erase, reshape, smooth, replace, or regenerate permanent human anatomy'));
  assert.ok(creator.includes('Do not beautify, normalize, retouch, symmetrize, or reconstruct a face'));
  assert.ok(creator.includes('add one small but obvious harmless accessory or decorative object rather than modifying anatomy'));
  assert.ok(!creator.includes('A removed human feature'));
  assert.ok(creator.includes('Never add, remove, replace, or alter a mustache'));
  assert.ok(creator.includes('Perform exactly this one requested difference: ${instruction}'));
  ...
  assert.ok(creator.includes('This uploaded image contains exactly the user-selected area'));
  assert.ok(creator.includes('The entire image is the editable target'));
  assert.ok(creatorHtml.includes('creator.js?v=19'));
