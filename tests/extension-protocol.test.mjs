...
test('creator and adapter include recovery guards for stalled website automation', () => {
  ...
  assert.ok(!creator.includes('AUTO_EDIT_VARIANTS'));
  assert.ok(!creator.includes('CREATIVE DIRECTION'));
  assert.ok(!creator.includes('automaticStyle'));
  assert.ok(creator.includes('Make exactly one clear, fun, reality-changing difference'));
  assert.ok(creator.includes('when this crop is placed back into a larger image'));
  assert.ok(creator.includes('Choose the change that best fits the crop'));
  assert.ok(creator.includes('faces, expressions, hair, anatomy, objects, and scenery are valid targets'));
  assert.ok(creator.includes('Do not add stickers, labels, or graphic overlays'));
  assert.ok(creator.includes('Keep the crop framing and all unrelated content unchanged'));
  ...
  assert.ok(adapter.includes('Make exactly one clear, fun, reality-changing difference'));
  assert.ok(adapter.includes('Do not add stickers, labels, or graphic overlays'));
